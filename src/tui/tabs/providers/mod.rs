...
impl ProvidersTab {
    pub fn new(mgr: Rc<ConfigManager>, app: AppType) -> Self {
        ...
        let selected_provider_idx = providers.iter().position(|p| p.id == active_provider).unwrap_or(0);
        ...
        let codex_models = providers.get(selected_provider_idx).map(|provider| provider.models.clone()).unwrap_or_default();

        let selected_profile_idx = if app == AppType::Codex {
            codex_models.iter().position(|model| model.slug == active_codex_model).unwrap_or(0)
    ...
    pub fn switch_app(&mut self, app: AppType) {
        ...
        self.active_codex_model = self.mgr.get_setting("active_codex_model").unwrap_or_default();
    ...
    pub fn status_text(&self) -> String {
        ...
        let provider = self.selected_provider().map(|p| p.name.as_str()).unwrap_or("-");
        if self.app == AppType::Claude {
            let profile = self.selected_profile().map(|p| p.name.as_str()).unwrap_or("-");
    ...
    fn commit_provider(&mut self) {
        ...
        if !form.is_edit && self.providers.iter().any(|provider| provider.id == pr.id) {
            self.message = Some(lang::pick_owned(format!("Provider '{}' already exists", pr.id), format!("供应商 '{}' 已存在", pr.id)));
            return;
        }
        if form.is_edit
                ...
                .is_some_and(|provider| provider.codex_catalog != pr.codex_catalog)
        {
            self.message = Some(lang::pick_owned(
                "Switch to another Codex provider before changing the active provider's catalog type".into(),
    ...
    fn load_profiles(&mut self) {
        ...
        let selection_len = if self.app == AppType::Codex { self.codex_models.len() } else { self.profiles.len() };
        if self.selected_profile_idx >= selection_len {
            self.selected_profile_idx = 0;
        }
        self.profile_state.select(if selection_len == 0 { None } else { Some(self.selected_profile_idx) });
    }
    ...
    fn do_add_codex_model(&mut self) {
        ...
        self.codex_model_form = Some(CodexModelForm {
            fields: [String::new(), String::new(), String::new(), "128000".into(), String::new(), "95".into()],
            cursors: [0, 0, 0, 6, 0, 2],
            focused: 0,
            is_edit: false,
    ...
    fn do_edit_codex_model(&mut self) {
        ...
        let fields = [
            ...
            model.max_context_window.map(|value| value.to_string()).unwrap_or_default(),
            model.effective_context_window_percent.to_string(),
        ];
        let cursors = std::array::from_fn(|index| fields[index].len());
        let mut supported_efforts = std::array::from_fn(|index| model.supported_reasoning_efforts.iter().any(|value| value == form::REASONING_EFFORTS[index]));
        if !supported_efforts.iter().any(|selected| *selected) {
            supported_efforts[1] = true;
        }
        let default_effort = form::REASONING_EFFORTS
            .iter()
            .position(|value| *value == model.default_reasoning_effort)
            .filter(|index| supported_efforts[*index])
            .or_else(|| supported_efforts.iter().position(|selected| *selected))
            .unwrap_or(1);
    ...
    fn commit_codex_model(&mut self) {
        ...
        let mut prospective = self.providers.clone();
        if let Some(provider) = prospective.iter_mut().find(|provider| provider.id == provider_id) {
            if model.default {
    ...
    fn show_catalog_status(&mut self) {
        ...
        let custom = providers.iter().filter(|provider| provider.codex_catalog == CodexCatalog::Custom).collect::<Vec<_>>();
        let models = custom.iter().map(|provider| provider.models.len()).sum::<usize>();
    ...
    fn preview_codex_model(&mut self) {
        if let Some(model) = self.selected_codex_model() {
            self.content_popup = Some(ContentPopup {
                ...
                content: serde_json::to_string_pretty(&model_entry(model)).unwrap_or_else(|error| error.to_string()),
                compact: false,
                scroll: 0,
                max_scroll: 0,
                page_height: 1,
    ...
    fn render_confirm_popup(&self, f: &mut Frame, area: Rect) {
        let (title, msg, c) = match self.confirm_action {
            ...
            Some(ProviderAction::Switch) => {
                if self.app == AppType::Codex {
                    (lang::current().confirm_switch_provider, lang::current().confirm_switch_provider_msg, theme::current().cyan)
                } else {
                    (lang::current().confirm_switch_profile, lang::current().confirm_switch_profile_msg, theme::current().cyan)
                }
            }
            _ => return,
        };
    ...
    fn render_content_popup(&mut self, f: &mut Frame, area: Rect) {
        ...
        let content_width = popup_state.content.lines().map(super::super::widgets::shared::display_width).max().unwrap_or(1);
        let width = if popup_state.compact {
            (content_width as u16).saturating_add(6).clamp(32, 72).min(area.width)
        } else {
            area.width.saturating_sub(4).clamp(32, 100).min(area.width)
        };
        let available_content_width = width.saturating_sub(6).max(1) as usize;
        let wrapped_line_count = wrap_display_lines(&popup_state.content, available_content_width).len().min(u16::MAX as usize) as u16;
        let height = if popup_state.compact {
            wrapped_line_count.saturating_add(3).clamp(6, 14).min(area.height)
        } else {
        ...
        popup_state.max_scroll = lines.len().saturating_sub(content_area.height as usize).min(u16::MAX as usize) as u16;
        popup_state.scroll = popup_state.scroll.min(popup_state.max_scroll);

        clear_popup_area(f, popup);
        f.render_widget(block, popup);
        f.render_widget(
            Paragraph::new(lines.into_iter().map(Line::from).collect::<Vec<_>>()).scroll((popup_state.scroll, 0)),
            content_area,
        );
        f.render_widget(
            Paragraph::new(Line::from(Span::styled(
                if popup_state.compact {
                    ...
                    lang::pick("J/K or Up/Down scroll · PgUp/PgDn page · Esc close", "J/K 或上下键滚动 · PgUp/PgDn 翻页 · Esc 关闭")
                },
    ...
    fn render_selection_detail(&self, f: &mut Frame, area: Rect) {
        ...
        if let Some(provider) = self.selected_provider() {
            lines.push(detail_line("Provider", &provider.name, theme::current().cyan));
            lines.push(detail_line("ID", &provider.id, theme::current().fg));
            lines.push(detail_line("Base URL", &provider.api_url, theme::current().yellow));
            ...
            if self.app == AppType::Codex {
                ...
                lines.push(detail_line("Wire API", "responses", theme::current().purple));
                lines.push(detail_line("Catalog", provider.codex_catalog.as_str(), theme::current().purple));
                lines.push(detail_line("Models", &provider.models.len().to_string(), theme::current().fg));
                if let Some(model) = self.selected_codex_model() {
                    lines.push(Line::from(""));
                    lines.push(detail_line("Model", &model.display_name, theme::current().cyan));
                    lines.push(detail_line("Slug", &model.slug, theme::current().fg));
                    lines.push(detail_line("Context", &format_context(model.context_window), theme::current().yellow));
                    lines.push(detail_line("Reasoning", &model.supported_reasoning_efforts.join("/"), theme::current().purple));
                    lines.push(detail_line("Default", &model.default_reasoning_effort, theme::current().fg));
                    lines.push(detail_line("Modalities", &model.input_modalities.join(", "), theme::current().fg));
                }
                ...
                lines.push(detail_line("Subagent", &profile.subagent, theme::current().fg));
            }
        } else {
            lines.push(Line::from(Span::styled("No provider selected", Style::default().fg(theme::current().comment))).centered());
        }
    ...
    fn render_codex_model_list(&mut self, f: &mut Frame, area: Rect) {
        ...
        let block = Block::bordered()
            .border_set(ratatui::symbols::border::ROUNDED)
            .title(format!("{} ({})", lang::current().models_title, self.codex_models.len()))
            .border_style(Style::default().fg(if focused { theme::current().cyan } else { theme::current().dim }));
        if self.selected_provider().is_some_and(|provider| provider.codex_catalog == CodexCatalog::BuiltIn) {
            f.render_widget(
                Paragraph::new(vec![
                    Line::from(""),
                    Line::from(Span::styled(
                        lang::pick("Uses the Codex built-in model catalog", "使用 Codex 内置模型目录"),
        ...
        let items = self
            ...
            .map(|(index, model)| {
                ...
                let active = self.active_provider == self.selected_provider().map(|p| p.id.as_str()).unwrap_or("") && self.active_codex_model == model.slug;
                ListItem::new(vec![
                    Line::from(vec![
                        Span::styled(
                            format!("{}{}", if selected { "› " } else { "  " }, model.display_name),
                            Style::default().fg(if selected { theme::current().cyan } else { theme::current().fg }),
                        ),
                        if active {
                    ...
                    Line::from(vec![
                        ...
                        Span::styled(format_context(model.context_window), Style::default().fg(theme::current().comment)),
                        Span::styled(" · ", Style::default().fg(theme::current().dim)),
                        Span::styled(&model.default_reasoning_effort, Style::default().fg(theme::current().comment)),
                    ]),
                    Line::from(""),
                ])
            })
            .collect::<Vec<_>>();
        f.render_stateful_widget(List::new(items).block(block).highlight_style(Style::default()), area, &mut self.profile_state);
    }
}

fn detail_line(label: &str, value: &str, color: Color) -> Line<'static> {
    Line::from(vec![
        Span::styled(format!("  {:<10}", format!("{}:", label)), Style::default().fg(theme::current().purple)),
    ...
    fn render(&mut self, f: &mut Frame, area: Rect) {
        ...
        let (provider_area, profile_area, detail_area) = if self.app == AppType::Codex {
            ...
            let [providers, profiles, detail] = Layout::default()
                .direction(Direction::Vertical)
                .constraints([Constraint::Percentage(34), Constraint::Percentage(33), Constraint::Min(9)])
                .areas(area);
            (providers, profiles, detail)
        };
        ...
        let profile_items: Vec<ListItem> = self
            ...
            .map(|(i, pr)| {
                ...
                let arrow = if is_sel { "› " } else { "  " };
                let tc = if is_sel { theme::current().cyan } else { theme::current().fg };
                let active = self.selected_provider().map(|p| p.id.as_str()) == Some(self.active_provider.as_str()) && self.active_profile == pr.id;
    ...
    fn handle_key(&mut self, code: KeyCode) -> bool {
        if let Some(popup) = self.content_popup.as_mut() {
            match code {
                ...
                KeyCode::Char('j') | KeyCode::Down => popup.scroll = popup.scroll.saturating_add(1).min(popup.max_scroll),
                KeyCode::Char('k') | KeyCode::Up => popup.scroll = popup.scroll.saturating_sub(1),
                KeyCode::PageDown => popup.scroll = popup.scroll.saturating_add(popup.page_height).min(popup.max_scroll),
        ...
        if self.confirm_action.is_some() {
            match code {
                KeyCode::Tab | KeyCode::Right => self.confirm_button = (self.confirm_button + 1) % 2,
                KeyCode::BackTab | KeyCode::Left => self.confirm_button = if self.confirm_button == 0 { 1 } else { 0 },
                KeyCode::Enter => {
        ...
        if self.app == AppType::Codex {
            return match self.panel {
                ...
                Panel::ProfileList => vec![
                    vec![(" J/K ".into(), theme::current().comment), (lang::current().sc_nav.into(), theme::current().comment)],
                    vec![(" H/← ".into(), theme::current().comment), (lang::current().sc_back.into(), theme::current().comment)],
                    vec![(" ⏎  ".into(), theme::current().comment), (lang::current().sc_switch.into(), theme::current().comment)],
                    vec![(" A ".into(), theme::current().comment), (lang::current().sc_add.into(), theme::current().comment)],
                    vec![(" E ".into(), theme::current().comment), (lang::current().sc_edit.into(), theme::current().comment)],
                    vec![(" D ".into(), theme::current().comment), (lang::current().sc_delete.into(), theme::current().comment)],
                    vec![(" C ".into(), theme::current().comment), (lang::pick("Catalog", "目录").into(), theme::current().comment)],
                    vec![(" V ".into(), theme::current().comment), (lang::pick("Preview", "预览").into(), theme::current().comment)],
                ],
            };
        }
        match self.panel {
            Panel::ProviderList => vec![
                vec![(" J/K ".into(), theme::current().comment), (lang::current().sc_nav.into(), theme::current().comment)],
                vec![(" L/→ ".into(), theme::current().comment), (lang::current().sc_profiles.into(), theme::current().comment)],
                vec![(" A ".into(), theme::current().comment), (lang::current().sc_add.into(), theme::current().comment)],
                vec![(" E ".into(), theme::current().comment), (lang::current().sc_edit.into(), theme::current().comment)],
                vec![(" D ".into(), theme::current().comment), (lang::current().sc_delete.into(), theme::current().comment)],
                vec![(" Q ".into(), theme::current().comment), (lang::current().sc_quit.into(), theme::current().comment)],
...
impl ProvidersTab {
    fn handle_provider_keys(&mut self, code: KeyCode) -> bool {
        match code {
            ...
            KeyCode::Enter => {
                if self.app == AppType::Codex {
                    match self.selected_provider().map(|provider| provider.codex_catalog) {
                        ...
                        Some(CodexCatalog::Custom) => {
                            ...
                            self.profile_state.select(if self.codex_models.is_empty() { None } else { Some(self.selected_profile_idx) });
                        }
                        None => {}
                    }
                } else {
            ...
            KeyCode::Char('l') | KeyCode::Right => {
                ...
                self.profile_state.select(if empty { None } else { Some(self.selected_profile_idx) });
            }
    ...
    fn handle_codex_model_keys(&mut self, code: KeyCode) -> bool {
        match code {
            ...
            KeyCode::Char('k') | KeyCode::Up => {
                let len = self.codex_models.len();
                if len > 0 {
                    self.selected_profile_idx = if self.selected_profile_idx == 0 { len - 1 } else { self.selected_profile_idx - 1 };
                    self.profile_state.select(Some(self.selected_profile_idx));
                }
            }
            KeyCode::Enter => {
                let built_in = self.selected_provider().is_some_and(|provider| provider.codex_catalog == CodexCatalog::BuiltIn);
...
fn localized_error(error: &anyhow::Error) -> String {
    if let Some(error) = error.downcast_ref::<crate::core::env::ApiKeyUnavailable>() {
        return lang::pick_owned(
            format!("Error: {}", error),
            format!("错误：供应商 '{}' 的 API 密钥不可用。请设置 {}，或使用明文密钥。", error.provider_id, error.env_var),
        );
    }
...
fn compact_model_name(model: &str) -> String {
    ...
    if model.is_empty() {
        ...
        model.replace("[1m]", " 1M").split_whitespace().collect::<Vec<_>>().join(" ")
    }
}
